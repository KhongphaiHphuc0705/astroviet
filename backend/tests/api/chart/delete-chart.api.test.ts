...
describe('DELETE /api/v1/charts/:id', () => {
  ...
  it('should return 204 and delete the chart if the user is the owner', async () => {
    ...
    // Verify it is actually soft-deleted (fetch should return 404)
  ...
  it('should return 401 if the user is not authenticated', async () => {
    const response = await request(app).delete(`/api/v1/charts/${chartId}`);
  ...
  it('should return 400 if the chart ID is not a valid UUID', async () => {
    const response = await request(app)
      .delete(`/api/v1/charts/not-a-uuid`)
      .set('Authorization', `Bearer ${accessToken}`);
